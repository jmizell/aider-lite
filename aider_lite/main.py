...
from aider_lite import __version__, models, urls, utils
from aider_lite.args import get_parser
from aider_lite.coders import Coder
from aider_lite.commands import Commands, SwitchCoder
from aider_lite.format_settings import format_settings, scrub_sensitive_info
from aider_lite.history import ChatSummary
from aider_lite.io import InputOutput
from aider_lite.llm import apiclient  # noqa: F401; properly init litellm on launch
from aider_lite.repo import ANY_GIT_ERROR, GitRepo
from aider_lite.report import report_uncaught_exceptions
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning("You should probably run aider_lite in a directory, not your home dir.")
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track aider_lite's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        repo = git.Repo(git_root)
        if repo.ignored(".aider_lite") and repo.ignored(".env"):
            return
    except ANY_GIT_ERROR:
        pass

    patterns = [".aider_lite*", ".env"]
    patterns_to_add = []
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".aider_lite.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading aider_lite model settings: {e}")
        return 1
...
def register_model_metadata(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("aider_lite.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    # Add user-specified files
    model_metadata_files += generate_search_path_list(
        ".aider_lite.model.metadata.json",
        git_root,
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        ...
        io.tool_output("Or run aider_lite --no-git to proceed without using git.")
        return False
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fname = Path(".aider_lite.conf.yml")
    ...
    io.tool_output('Use /help <question> for help, run "aider_lite --help" to see cmd line args')
...
def check_and_load_imports(io, verbose=False):
    installs_file = Path.home() / ".aider_lite" / "installs.json"

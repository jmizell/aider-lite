...
from aider_lite import models, prompts
from aider_lite.format_settings import format_settings
from aider_lite.llm import apiclient
from aider_lite.repo import ANY_GIT_ERROR
from aider_lite.run_cmd import run_cmd
from aider_lite.utils import is_image_file
...
class Commands:
    ...
    def cmd_chat_mode(self, args):
        ...
        show_formats = OrderedDict(
            [
                ("help", "Get help about using aider_lite (usage, config, troubleshoot)."),
    ...
    def cmd_tokens(self, args):
        ...
        if remaining > 1024:
            ...
            self.io.tool_error(
                f"{fmt(remaining)} tokens remaining in context window (use /drop or"
                " /clear to make space)"
            )
        else:
            self.io.tool_error(
                f"{fmt(remaining)} tokens remaining, window exhausted (use /drop or /clear to make space)"
            )
        self.io.tool_output(f"{fmt(limit)} tokens max context window size")

    def cmd_undo(self, args):
        "Undo the last git commit if it was done by aider_lite"
        try:
    ...
    def raw_cmd_undo(self, args):
        ...
        if last_commit_hash not in self.coder.aider_commit_hashes:
            self.io.tool_error("The last commit was not made by aider_lite in this chat session.")
            self.io.tool_output(
    ...
    def cmd_add(self, args):
        "Add files to the chat so aider_lite can edit them or review them in detail"
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use aider_lite.")
